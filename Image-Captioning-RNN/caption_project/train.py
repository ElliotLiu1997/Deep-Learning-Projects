...
def _build_dataloaders(cfg, vocab: Vocabulary):
    ...
    train_ds = Flickr8kCaptionDataset(
        ...
        feature_npy_path=cfg.features_train_path if cfg.use_feature_cache else None,
        feature_keys_json_path=cfg.feature_keys_train_path if cfg.use_feature_cache else None,
    ...
    val_ds = Flickr8kCaptionDataset(
        ...
        max_caption_length=cfg.max_caption_length,
        use_feature_cache=cfg.use_feature_cache,
        feature_npy_path=cfg.features_val_path if cfg.use_feature_cache else None,
        feature_keys_json_path=cfg.feature_keys_val_path if cfg.use_feature_cache else None,
        image_transform=None if cfg.use_feature_cache else image_transform,
    )
...
def _forward_model(model: ImageCaptioningModel, x: torch.Tensor, input_seq: torch.Tensor, use_feature_cache: bool):
    ...
    return model.forward_images(x, input_seq)
...
def train_single_variant(cfg, vocab: Vocabulary, variant: str, train_dl: DataLoader, val_dl: DataLoader):
...
def main():
    ...
    train_dl, val_dl = _build_dataloaders(cfg, vocab)

    all_histories: Dict[str, Dict] = {}
    for variant in cfg.model_variants:
        history = train_single_variant(cfg, vocab, variant, train_dl, val_dl)
